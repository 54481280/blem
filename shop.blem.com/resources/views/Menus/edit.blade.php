            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <form action="{{route('menus.update',[$menu])}}" method="post">
                        ...
                        <div>
                            <input type="text" class="form-control" name="name" value="{{$menu->name}}" placeholder="请输入菜品分类名称">
                        </div>
                        <div class="sub-title">菜品编号</div>
                        <div>
                            <input type="text" class="form-control" name="type_accumulation" value="{{$menu->type_accumulation}}" placeholder="请输入菜品编号">
                        </div>
                        <div class="sub-title">是否默认分类</div>
                        <div>
                            <div class="radio3 radio-check radio-success radio-inline">
                                <input type="radio" id="radio5" name="is_selected" value="1" @if($menu->is_selected) checked @endif>
                                <label for="radio5">
                                    是
                                </label>
                            </div>
                            <div class="radio3 radio-check radio-inline">
                                <input type="radio" id="radio6" name="is_selected" value="0" @if(!$menu->is_selected) checked @endif>
                                <label for="radio6">
                                    否
                                </label>
                            </div>
